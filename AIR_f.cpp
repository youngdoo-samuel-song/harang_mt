...
PI_THREAD(GPS_THREAD) {  // GPS ë°ì´í°ë¥¼ sdì¹´ëì ì ì¥
    ...
    while(1){
        if (serialDataAvail(gps_serial_port)) {
            ...
            gpsout << time_stamp << endl << gps_data << endl << endl;
            //cout << gps_data;
        }
    }
...
PI_THREAD(MAIN_THREAD) {  // ë©ì¸ í¨ì, ëíì° ì¬ì¶ ìí, ì¬ì¶ ê´ë ¨ ë°ì´í°ë¥¼ sdì¹´ëì ì ì¥
    ...
    while(1) {
        ...
        // SDì¹´ë ì ì¥
        fout << time_stamp << ' ' << on << ' ' << altitude << ' ' << max_altitude << ' ' << step << endl;

        // ë°ì´í° ì¶ë ¥
        cout << time_stamp << ' ' << on << ' ' << altitude << ' ' << max_altitude << ' ' << step << endl;
